...
library(janitor) # helper functions for cleaning data frames
...
transplant <- read_csv("data/raw/transplant_raw.csv", na = c("", "NA", "na", " "))

## option 2: if excel, use readxl(). ---------
# note- load this version too to use to see differences in how read.csv and readxl 
# work, but after we do that, the rest of code is set up to work with the read_csv version
transplant_excel <- read_excel("data/raw/transplant_raw.xlsx", 
                          sheet = 1, col_names = T, col_types = NULL) 
# col_types = null is default, and readxl guesses what each column is. 
# by default, readxl converts blank cells to missing data. 
# use na="yourcode" if you have a code you use for missing values.
...
str(transplant_excel) # most variables are character class. dates are read in 
# intelligently, though, although it added the current year because year was missing.

summary(transplant) #gives summary information, which can help you identify if 
# any columns are in the wrong class or if there are data entry errors or a lot 
# of NA's etc.
glimpse(transplant) #like summary but shows more data - perhaps you like this better? 
names(transplant) #show column names


# **Side note: Introduction to piping** -------------
# Each function returns an object, so calls can be chained together in a single 
# statement, without needing variables to store the intermediate results.
# it takes the output of one statement and makes it the input of the next statement. 
# When reading the code, you can think of the piping symbol as saying "AND THEN".

# |> is base piping function. (%>% from magrittr package also works, but tidyverse 
# recommends |> now)

transplant |>
  filter(Island == "Guam") |>
  summarise(meanwebsize = mean(`WebSize(cm)`, na.rm = TRUE))

# the code chunk above will translate to something like "you take the transplant data, 
# then you subset the Guam data and then you calculate the mean websize.

#non-piping approach, using square brackets to subset
summarize(transplant[transplant$Island == "Guam",], 
          meanwebsize = mean(`WebSize(cm)`, na.rm = TRUE))
...
# as opposed to from inside and out;
...
# 3.0 Clean column names using clean_names function in janitor package
transplant <- transplant |> 
                clean_names() 

# renames columns 

# 3.1: Rename column headings using rename in dplyr
# new name is on left, old name is on right.
transplant <- transplant |> 
  rename(WebSize = web_size_cm)

colnames(transplant) #note that WebSize has uppercase letters; I will return to that next

# 3.2: Fix upper case/lower case issues in column names
#change column names from upper to lowercase

transplant <- transplant |>
  rename_with(tolower) #rename_with() renames all columns using a function like "tolower"
...
transplant <- transplant |>
  select(-total_days) #check to make sure you have 1 less column than before (back to 11 variables)

colnames(transplant)

## 4.3: Separate one column into two columns ------
transplant <- transplant |>
  separate(col = web_number, into = c("web_a", "web_b"), sep = "'", remove = FALSE) 
...
transplant <- transplant |>
  unite(start_date2, c(start_date, year), sep="-", remove = FALSE)
#this creates a new startdate column with the new value, and leaves the year column (now 14 columns)

transplant <- transplant |>
  unite(end_date2, c(end_date, year), sep="-", remove = FALSE)
#this creates a new enddate column, and removes the Year column, so still at 14 columns. 
...
transplantwide <- transplant |>
  pivot_wider(names_from = web_pres, values_from = websize) 

#the info in webpres will be the new column names, and the columns will be populated by the values in websize

# If you want to automatically add in a number for new rows with NA, can use argument "values_fill = 0", and it'll put a 0 there. 
...
transplant_long <- transplantwide |>
  pivot_longer(cols = c(no, yes), names_to = "webpres", values_to = "websize") 

# this makes a new column called "webpres" populated by "no" or "yes", and then another new column called "websize" populated by the values within the former "no" and "yes" columns. 
...
# if you want to drop the NA values, use values_drop_na = T
transplant_long <- transplantwide |>
  pivot_longer(cols = c(no, yes), names_to = "webpres", values_to = "websize", values_drop_na = TRUE)
...
transplant <- transplant |>
  mutate(across(c(island, site, web_number, native, netting, spid_pres), as.factor)) #use across to apply a function across multiple columns
...
# returns 4133 obs of 17 variables
...
# Returns 4238 obs of 17 variables
