...
# Your turn 4: Calculate the mean, min, and max duration for spiders on Saipan. 
...
# Your Turn 3: Select columns web_a, web_b, webarea, native, netting, and duration, and arrange them so that native, netting, and duration are the first three columns. 
transplant2 <- transplant |>
  select(native, netting, duration, web_a, web_b)
...
# Your turn 5: Calculate the mean, median, min, and max duration for webs with and without netting on Guam and Saipan, and calculate the number of webs in each group
  ...
  summarize(mean_dur = mean(duration, na.rm = T), 
            med_dur = median(duration, na.rm = T), 
            min_dur = min(duration, na.rm = T), 
            max_dur = max(duration, na.rm = T), 
            nwebs = n())
...
transplant2 <- transplant |>
  ...
  mutate(mean_dur = mean(duration),
         diff_mean_duration = duration - mean_dur) |>
  select(island, site, netting, web, websize, duration, mean_dur, diff_mean_duration)
