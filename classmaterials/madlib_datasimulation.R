...
wolverine_body <- rnorm(n, mean = mean_size, sd = sd_size)
help(rnorm)

#Randomly assign each wolverine cave entrance presence (Y/N)



# Combine into a tibble
sim <- tibble(id = 1:n, wolverine_body = wolverine_body)

# Look at the data
print(sim)
summary(sim$wolverine_body) #mean of 30.23

# Visualize with a histogram
ggplot(sim, aes(x = wolverine_body)) +
  geom_histogram(binwidth = 0.5, fill = "skyblue", color = "black") +
  labs(title = "Simulated cave wolverine body sizes", x = "Body size (Ibs)", y = "Count") +
  theme_classic()

# =====================================================
# 2. Add a treatment effect (cave entrance)
# =====================================================
...
# Create a treatment variable (half yes, half no)
n <- 50
treatment <- rep(c("yes", "no"), each = n/2)
...
wolverine_body <- rnorm(
  n = n,
  treatmentmean = if_else(treatment == "yes", mean_yes, mean_no), #if the treatment is trt, then use the mean_trt. If not trt, use the mean_control for the mean
  sd = sd_both
)

# Combine into tibble
(sim2 <- tibble(treatment, wolverine_body))

# Visualize
ggplot(sim2, aes(x = treatment, y = body_size)) +
  geom_boxplot(fill = "lightgreen") +
  labs(title = "Simulated wolverine body sizes by cave entrance",
       x = "Cave Entrance presence", y = "Body size (pounds)") +
  theme_classic()

# Quick summaries
sim2 |>
  group_by(treatment) |>
  summarise(mean_size = mean(wolverine_body), sd_size = sd(wolverine_body), n = n())

# =====================
# Add a treatment effect (canopy coverage)
